...
recruitment already has a system of record — call it **Careers** — that owns the
...
So v2 gives up the part it was never going to win, and keeps the part Careers
cannot do:

| | Careers owns | Growth owns |
|---|---|---|
| The requisition | ✓ | |
...
**Growth becomes the top of the vacancy funnel and proves it. Careers keeps the
...
nightly read from the Careers requisition feed.
...
```
  Careers (system of record)
     │  requisition exists
     ▼
  ┌──────────────────────────────────────────────┐
  │  Growth · Internal Vacancies board            │
  │  · match score, skills, fit breakdown         │   ← the value Careers cannot add
  │  · "View & apply in Careers ↗"                │
  └──────────────────┬───────────────────────────┘
                     │  generates GRW-K7P2QX, writes v2_vacancy_referrals
                     │  opens  …/job/REQ-2481?src=growth&gref=GRW-K7P2QX
                     ▼
  ┌──────────────────────────────────────────────┐
  │  Careers · the application actually happens   │
  ...
  └──────────────────┬───────────────────────────┘
        ...
        ┌────────────┴────────────┐
        ▼                         ▼
  Growth asks:              Monthly: join the Careers
...
| **From Careers** | confirmed applications, hires | the monthly export join | The only figures fit for a board paper. |
...
| Referrals to Careers | `v2_vacancy_referrals` — one row per handoff click | measured |
| Click-through rate | referrals ÷ views | measured |
| Said they applied | referrals where `outcome = 'applied'` | self-reported |
| Confirmed in Careers | referrals where `verified = true` | from Careers |
| Confirmed rate | confirmed ÷ referrals — *the honesty ratio, see §7* | from Careers |
| Hires attributed | confirmed applications that became offers | from Careers / HR |
...
*M* of those applications are confirmed in Careers."
...
2. **Paste `GRW-K7P2QX` into "how did you hear about this role?" in Careers.**
...
inside Careers. **That is precisely why it has to ask.**
...
2. **Get the Careers export** of internal applications for the same period,
...
Ask them to add a source value **`Growth Marketplace`** to the Careers
...
**Later, if volume justifies it:** a Careers webhook posting `application.created`
...
some of them read the same role in the careers newsletter that morning.
...
> on the marketplace and leave the other half listed only in Careers. Compare
...
applications are Careers', and the word **"because"** belongs to neither of us yet.
...
**Not real, and labelled as such in the UI:** no requisition feed from Careers —
...
4. **What happens to the DJI "for job consideration" path** under Policy
   ...
   one place where the marketplace and Careers genuinely overlap, and v2 does not
   resolve it.
