...
This model is built on tab v1 of ALP_Computations.xlsx: backfill-netted
hiring costs, salary-priced vacancy days, and an avoided-turnover benefit.
Tab v2 replaced all three — no backfill deduction, vacancy priced as lost
revenue, and no turnover benefit at all. The live model is docs/exec-model.py,
which reproduces tab v2 and checks itself against it.

Kept only so the earlier figures in circulation can be traced.
"""
